...
//Fungsi yang mengembalikan indeks angka 1 paling kiri dari baris a
 ...
 public void solveCramerRule() {
        ...
        double[] konstantaPersamaan = new double[this.Kol + 1];
        Matriks koefisienVariabel = new Matriks(this.Brs, this.Kol - 1);

        for (int i = 1; i <= this.Brs; i++) {
            for (int j = 1; j <= this.Kol; j++) {
                if (j == this.Kol) {
                    konstantaPersamaan[i] = this.Mat[i][j];
                } else {
                    koefisienVariabel.data[i][j] = this.Mat[i][j];
                }
            }
        }
        ...
        for (int kolomTukar = 1; kolomTukar <= this.Kol; kolomTukar++) {
            Matriks tempMatrix = new Matriks(koefisienVariabel.Brs, koefisienVariabel.Kol;
